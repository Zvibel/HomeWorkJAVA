package HomeWork.HomeWorkOOP.HW4.homework;
...
public class Box<F extends Fruit> {
    ...
    public void addFruitToBox(Fruit fruits) {
        if (name.equals(fruits.getNAME()) || name.equals("Empty box") ||
            ...
            System.out.println("Недопустимая операция вы пытаетесь положить фрукты не в ту коробку!");
    ...
    public void transferFruitThisBoxToBox(Box<F> toBox, int countMove) {
        if (countMove > fruits.size() || !name.equals(toBox.name) && !toBox.name.equals("Empty box")) return;
        else {
            ...
            this.sumOfWeigt = getWeihtOfBox();
            toBox.getWeihtOfBox();
            if (fruits.size() == 0) name = "Empty box for " + toBox.name;
